let jsonHero = `[{
    ...
    "universe": "DC Comics",
    "alterego": "Брюс Уэйн",
    "occupation": "борец против преступности, филантроп, миллиардер",
    ...
    "info": "По популярности человек-летучая мышь может сравниться только  Суперменом. Образ кажется очень мистическим и мрачным: черный костюм с развевающимся плащом, устрашающий Бэтмобиль, штаб-квартира в сырой пещере. Его биография настолько же темна, как и образ. В детстве у него на глазах убили родителей, и юный Брюс поклялся всеми силами защищать родной Готэм от преступности. Брюс знаменит, несмотря на пожилой возраст: первый выпуск с его участием вышел аж в 1939 году. С каждым годом популярность только крепнет. Большую роль в этом сыграли экранизации: «Темный рыцарь», сериал «Готэм» и несколько одноименных картин. Главная причина такой любви среди фанатов — мысль, что каждый может стать Бэтменом. Если подумать, это один из немногих героев, кто добился своего положения за счет ума и упорства, а не за счет мутаций."
},{
    "name": "Супермен",
    "universe": "DC Comics",
    "alterego": "Кларк Кент",
    ...
    "info": "Идею создания Халка как персонажа можно отнести к фантастической новелле «Странная история доктора Джекила и мистера Хайда», в которой ученый не может примириться со своим демоническим альтер эго. Прошлое у Халка — хуже некуда: отец Брюса страдал от алкоголизма и приступов ярости, а мать пыталась всеми силами уберечь сына от гнева своего мужа. Она погибла, защищая своего единственного ребенка от его побоев. Пронеся через все детство тяжелый психологический груз, Брюс вырос в чрезвычайно одаренного, но замкнутого юношу. Хотя Брюс не мог уважать отца, но пошел в науке по  стопам и выбрал ядерную физику. в течении время эксперимента он ввел пробную сыворотку и подверг себя воздействию большого количества гамма-радиации. Беннер превратился в мускулистого неуправляемого гиганта, который крушил все вокруг и не понимал, что происходит. Так имя Халка стало нарицательным для людей, который не могут держать себя в руках."
},{
    "name": "Чудо-женщина",
    "universe": "DC Comics",
    ...
    "info": "Как и Росомаха из Людей Икс, Дэдпул был подвергнут опытам по программе «Оружие Икс». Ученые попытались исцелить рак, привив клеткам способность к регенерации. Как и всегда в комиксах, что-то пошло не так, и Дэдпул остался изуродованным и психически нестабильным. Это единственный супергерой из списка, который однозначно не на стороне добра. Дэдпул наслаждается насилием. Первоначально появившись в основной Вселенной Marvel, он получил альтернативные варианты в других реальностях Мультивселенной. Что оставалось неизменным — циничное, чёрное чувство юмора: за него Дэдпула прозвали «Болтливым наёмником» "
}]`;

const container = document.querySelector('.container');

const superhero = JSON.parse(jsonHero);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    for (let hero of superhero) {
        ...
        superheroesContent += `<div class="superheroes">
            <div id="${heroId}" class="superheroes_card">
                <h2 class="name">${hero.name}</h2>
                <div class="image"><img class="hero_image" src="${hero.url}" alt="superheroImage"></div>
                <div class="rating" data-hero-id="${heroId}">
                    <span class="rating__star ${hero.rating >= 1 ? 'active' : ''}" data-value="1">★</span>
                    <span class="rating__star ${hero.rating >= 2 ? 'active' : ''}" data-value="2">★</span>
                    <span class="rating__star ${hero.rating >= 3 ? 'active' : ''}" data-value="3">★</span>
                    <span class="rating__star ${hero.rating >= 4 ? 'active' : ''}" data-value="4">★</span>
                    <span class="rating__star ${hero.rating >= 5 ? 'active' : ''}" data-value="5">★</span>
                </div>
                <div class="universe"><span>Вселенная:</span> ${hero.universe}</div>
                <div class="alterego"><span>Альтер эго:</span> ${hero.alterego}</div>
                <div class="occupation"><span>Род деятельности:</span> ${hero.occupation}</div>
                <div class="friends"><span>Друзья:</span> ${hero.friends}</div>
                <div class="superpowers"><span>Суперсилы:</span> ${hero.superpowers}</div>
                <div class="info">${hero.info}</div>
            </div>
        </div>`;
    }
    
    container.innerHTML = superheroesContent;

    const stars = document.querySelectorAll('.rating__star');

    function setRating(heroId, rating) {
        rating = Math.max(1, Math.min(5, rating));
        localStorage.setItem(`heroRating_${heroId}`, rating);
    }

    function getSavedRating(heroId) {
        return localStorage.getItem(`heroRating_${heroId}`);
    }
    ...
    function handleStarClick(event) {
        ...
        const heroId = star.closest('.superheroes_card').id;
        const rating = star.dataset.value;
        setRating(heroId, rating);
        setStarsActive(star.parentNode.querySelectorAll('.rating__star'), rating);
    }

    function updateRating(heroId) {
        const savedRating = getSavedRating(heroId);
        if (savedRating) {
          const rating = parseInt(savedRating);
          const ratingContainer = document.querySelector(`#${heroId} .rating`);
          setStarsActive(ratingContainer.querySelectorAll('.rating__star'), rating);
        }
      }
